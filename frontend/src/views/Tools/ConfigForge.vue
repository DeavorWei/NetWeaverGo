...
async function executeTaskSend() {
  ...
  try {
    ...
    const items = bindingPreview.value.map(
    ...
    showSendResult(
      true,
      `任务「${taskModal.value.name.trim()}」已发送到任务执行`,
      bindingPreview.value.length,
      [],
    );
  } catch (err: any) {
      ...
      <div v-if="sendModal.show" class="modal-container modal-active">
        ...
        <div class="modal modal-lg modal-glass">
          ...
          <div class="modal-footer">
            <button @click="closeSendModal" class="btn btn-secondary">
              取消
            </button>
            <button
              @click="executeSend"
        ...
        <div class="modal modal-lg modal-glass">
          ...
          <div class="modal-body space-y-5">
            ...
            <div class="space-y-1.5">
              ...
              <div class="flex flex-wrap gap-2 mb-2">
                <span
                  v-for="(tag, idx) in taskModal.tags"
                  :key="idx"
                  class="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-accent/10 text-accent border border-accent/20"
                >
                  {{ tag }}
                  <button
                    @click="removeTaskTag(idx)"
                    class="hover:text-error transition-colors"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
              ...
              <div class="flex gap-2">
                <input
                  v-model="taskModal.newTag"
                  type="text"
                  class="flex-1 px-3 py-2 rounded-lg bg-bg-secondary border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/50"
                  placeholder="添加标签"
                  @keyup.enter="addTaskTag"
                />
                <button
                  @click="addTaskTag"
                  class="px-3 py-2 rounded-lg bg-accent/10 border border-accent/30 text-accent text-sm font-medium hover:bg-accent/20 transition-colors"
        ...
        <div class="modal modal-lg modal-glass">
          ...
          <div class="modal-body text-sm text-text-secondary space-y-5">
            <p class="leading-relaxed text-base">
              <strong class="text-text-primary">ConfigForge</strong>
              是一款强大的自动化配置生成工具，所有核心计算逻辑均由后端处理，前端仅负责表单提交和结果渲染。
            </p>

            <div
              class="bg-accent-bg/50 rounded-xl p-5 border border-accent/20 shadow-inner"
            >
              ...
              <ul class="space-y-4">
                <li class="flex flex-col">
                  <span class="text-sm font-bold text-text-primary mb-1.5"
                    >1. Template Input (模板输入)</span
                  >
                  <span class="text-text-muted leading-relaxed"
                    >在左侧输入配置框架，使用
                    <code
                      class="px-1.5 py-0.5 bg-bg-tertiary rounded-md border border-border text-accent font-mono"
                      >[A]</code
                    >,
                    <code
                      class="px-1.5 py-0.5 bg-bg-tertiary rounded-md border border-border text-accent font-mono"
                ...
                <li class="flex flex-col">
                  <span class="text-sm font-bold text-text-primary mb-1.5"
                    >2. Variables Mapping (变量映射)</span
                  >
                  <span class="text-text-muted leading-relaxed"
                    >在中列输入变量值。后端自动处理<strong class="text-accent"
                      >语法糖展开</strong
                    >及<strong class="text-accent">等差数列补全</strong>。</span
                  >
                </li>
                <li class="flex flex-col">
                  <span class="text-sm font-bold text-text-primary mb-1.5"
                    >3. Output Preview (输出预览)</span
                  >
                  <span class="text-text-muted leading-relaxed"
                    >右侧实时显示后端生成的配置块，前端无需任何计算。</span
                  >
                </li>
              </ul>
            </div>
            ...
            <div
              class="flex items-start bg-info-bg rounded-xl p-4 border border-info/30 shadow-sm"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5 text-info mt-0.5 shrink-0 mr-2.5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <p class="text-sm text-info leading-relaxed">
                <strong>后端计算：</strong
                >变量替换、语法糖展开、等差数列推断、IP验证等核心逻辑均在后端执行，前端仅负责表单提交和结果渲染。
              </p>
            </div>
          </div>
        ...
        <div class="modal modal-lg modal-glass">
          ...
          <div class="modal-body text-sm text-text-secondary space-y-5">
            <p class="leading-relaxed">
              语法糖展开和等差数列推断由<strong class="text-accent"
                >后端服务</strong
              >处理。输入格式后失去焦点时，后端会自动展开。
            </p>
            ...
            >
              ...
              <ul class="space-y-4">
                <li class="flex flex-col">
                  <span class="text-xs font-medium text-text-muted mb-1.5"
                    >▶ 基础数字序列</span
                  >
                  <div class="flex items-center space-x-2.5">
                    <code
                      class="bg-bg-secondary px-2.5 py-1 rounded-lg border border-border text-text-primary font-mono"
                      >1-5</code
                    >
                    <span class="text-text-muted text-xs">➜</span>
                    <code
                      class="bg-bg-tertiary/80 px-2.5 py-1 rounded-lg border border-border text-text-secondary font-mono"
                      >1, 2, 3, 4, 5</code
                    >
                  </div>
                </li>
            ...
            <div
              class="bg-success-bg/50 rounded-xl p-5 border border-success/20 shadow-inner"
            >
              ...
              <p class="text-sm text-success leading-relaxed mb-3">
                当其他变量已有确定长度时，输入
                <strong>2个及以上</strong> 构成规律的数列，后端将自动补全对齐。
              </p>
            </div>
          </div>
