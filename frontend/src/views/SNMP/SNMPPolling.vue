...
<template>
  <div class="flex flex-col h-full">
    ...
    <header class="flex items-center justify-between px-4 py-3 bg-bg-secondary border-b border-border/50 flex-shrink-0">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          ></span>
          <span class="text-sm font-medium" :class="isRunning ? 'text-green-400' : 'text-text-muted'">
            {{ isRunning ? '运行中' : '已停止' }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-3 px-3 py-1.5 bg-bg-tertiary rounded-md text-xs">
          <span class="text-text-muted">目标: <span class="text-text-primary font-medium">{{ targetCount }}</span></span>
        ...
        >
          ...
          <div class="flex items-center justify-between px-4 py-2 bg-bg-secondary/50 border-b border-border/30 flex-shrink-0">
            <div class="flex items-center gap-3">
              ...
              >
                <option v-for="opt in targetEnabledOptions" :key="String(opt.value)" :value="opt.value">
                  {{ opt.label }}
                </option>
              </select>
            </div>
          ...
          <div class="flex-1 overflow-auto">
            <table class="w-full text-sm">
              <thead class="sticky top-0 bg-bg-secondary border-b border-border/50">
                <tr>
                  <th class="px-3 py-2 text-left text-text-muted font-medium w-8"></th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">名称</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">IP 地址</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">状态</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">最近轮询</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">延迟</th>
                  <th class="px-3 py-2 text-center text-text-muted font-medium">操作</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  :class="[
                    ...
                    selectedTarget?.id === target.id
                      ? 'bg-accent-bg/30'
                      : 'hover:bg-bg-hover',
                    isNewResult(target.id) ? 'animate-pulse bg-green-500/5' : ''
                  ]"
                >
                  <!-- 状态图标 -->
                  <td class="px-3 py-2">
                  ...
                  <!-- 名称 -->
                  <td class="px-3 py-2">
                    <span class="text-text-primary">{{ target.displayName }}</span>
                  </td>
                  <!-- IP 地址 -->
                  <td class="px-3 py-2">
                    <span class="text-text-secondary font-mono text-xs">{{ target.targetIP }}:{{ target.targetPort }}</span>
                  </td>
                  <!-- 状态 -->
                  <td class="px-3 py-2">
                  ...
                  <!-- 最近轮询时间 -->
                  <td class="px-3 py-2">
                    <span class="text-text-muted text-xs">{{ formatTime(target.lastPollAt) }}</span>
                  </td>
                  <!-- 延迟 -->
                  <td class="px-3 py-2">
                    <span v-if="getLatestResult(target.id)" class="text-text-secondary text-xs font-mono">
                ...
                <tr v-if="filteredTargets.length === 0">
                  <td colspan="7" class="px-3 py-8 text-center text-text-muted text-sm">
        ...
        >
          ...
          <div class="flex items-center justify-between px-4 py-2 bg-bg-secondary/50 border-b border-border/30 flex-shrink-0">
            <div class="flex items-center gap-3">
              ...
              <span v-if="resultFilter.targetId" class="text-xs text-text-muted">
                目标 #{{ resultFilter.targetId }}
                <button @click="resetResultFilter" class="ml-1 text-accent hover:underline">清除</button>
              </span>
            </div>
            ...
            <table class="w-full text-sm">
              <thead class="sticky top-0 bg-bg-secondary border-b border-border/50">
                <tr>
                  ...
                  <th class="px-3 py-2 text-left text-text-muted font-medium">目标 IP</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">OID</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">值</th>
                  <th class="px-3 py-2 text-left text-text-muted font-medium">类型</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  <td class="px-3 py-2">
                    <span class="text-text-secondary text-xs">{{ formatTime(result.pollTime) }}</span>
                  </td>
                  <td class="px-3 py-2">
                    <span class="text-text-primary text-xs font-mono">{{ result.targetIP }}</span>
                  </td>
                  <td class="px-3 py-2">
                    <span class="text-text-primary text-xs font-mono">{{ result.oid }}</span>
                    <span v-if="result.oidName" class="text-text-muted text-xs ml-1">({{ result.oidName }})</span>
                  </td>
                  <td class="px-3 py-2">
                    <span class="text-text-primary text-xs font-mono truncate max-w-[200px] block">{{ result.value || '-' }}</span>
                  </td>
                  <td class="px-3 py-2">
                    <span class="text-text-muted text-xs">{{ result.valueType || '-' }}</span>
                  </td>
                </tr>
                <tr v-if="pollingResults.length === 0">
                  <td colspan="5" class="px-3 py-6 text-center text-text-muted text-sm">
          ...
          <!-- 分页 -->
          <div v-if="totalPages > 1" class="flex items-center justify-between px-4 py-2 bg-bg-secondary/50 border-t border-border/30 flex-shrink-0">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-bg-secondary rounded-lg shadow-xl border border-border/50 w-[520px] max-h-[85vh] overflow-y-auto">
          ...
          <div class="p-5 space-y-4">
            ...
            <!-- SNMP 版本 -->
            <div>
              <label class="block text-sm text-text-muted mb-1">SNMP 版本 *</label>
              <select
            ...
            <div v-if="credentialForm.version !== 'v3'">
              <label class="block text-sm text-text-muted mb-1">Community String *</label>
              <input
            ...
            <!-- v3: 认证协议和密钥 -->
            <div v-if="credentialForm.version === 'v3' && credentialForm.securityLevel !== 'noAuthNoPriv'">
            ...
            <div v-if="credentialForm.version === 'v3' && credentialForm.securityLevel !== 'noAuthNoPriv'">
              <label class="block text-sm text-text-muted mb-1">认证密钥 {{ editingCredential ? '(留空保持原值)' : '*' }}</label>
              <input
            ...
            <div v-if="credentialForm.version === 'v3' && credentialForm.securityLevel === 'authPriv'">
            ...
            <div v-if="credentialForm.version === 'v3' && credentialForm.securityLevel === 'authPriv'">
              <label class="block text-sm text-text-muted mb-1">加密密钥 {{ editingCredential ? '(留空保持原值)' : '*' }}</label>
              <input
            ...
            <!-- v3: 上下文配置 (可选) -->
            <div v-if="credentialForm.version === 'v3'">
              <label class="block text-sm text-text-muted mb-1">上下文名称 (可选)</label>
              <input
            ...
            <div v-if="credentialForm.version === 'v3'">
              <label class="block text-sm text-text-muted mb-1">上下文引擎 ID (可选)</label>
              <input
          ...
          <div class="px-5 py-3 border-t border-border/30 flex justify-end gap-2">
            ...
            <button
              ...
              :disabled="!credentialForm.name || (credentialForm.version !== 'v3' && !credentialForm.community) || (credentialForm.version === 'v3' && !credentialForm.username)"
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-bg-secondary rounded-lg shadow-xl border border-border/50 w-[560px] max-h-[85vh] overflow-y-auto">
          <div class="px-5 py-4 border-b border-border/30">
            <h3 class="text-base font-medium text-text-primary">
          ...
          <div class="p-5 space-y-4">
            ...
            <div>
              ...
              <div class="space-y-2 max-h-[240px] overflow-y-auto">
                ...
                <div v-if="templateForm.oidItems.length === 0" class="text-center text-text-muted text-xs py-3">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-bg-secondary rounded-lg shadow-xl border border-border/50 w-[480px] max-h-[80vh] overflow-y-auto">
          <div class="px-5 py-4 border-b border-border/30">
            <h3 class="text-base font-medium text-text-primary">
          ...
          <div class="p-5 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-3">
              <div class="col-span-2">
                <label class="block text-sm text-text-muted mb-1">IP 地址 *</label>
                <input
            ...
            <div>
              ...
              <p v-if="credentials.length === 0" class="text-xs text-yellow-400 mt-1">请先创建凭据</p>
            </div>
            ...
            <div>
              ...
              <p v-if="templates.length === 0" class="text-xs text-yellow-400 mt-1">请先创建模板</p>
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm text-text-muted mb-1">轮询间隔（秒）</label>
                <input
          ...
          <div class="px-5 py-3 border-t border-border/30 flex justify-end gap-2">
            ...
            <button
              @click="saveTarget"
              :disabled="!targetForm.displayName || !targetForm.targetIP || !targetForm.credentialId || !targetForm.templateId"
              class="px-4 py-2 text-sm bg-accent hover:bg-accent-dark text-white rounded-md transition-colors disabled:opacity-50"
            >
              保存
            </button>
