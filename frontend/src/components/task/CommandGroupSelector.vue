<template>
  <div class="command-group-selector flex flex-col h-full">
    ...
    <div
      ...
      class="flex flex-col items-center justify-center py-4 text-text-muted gap-2 flex-1"
    >
      <svg
    ...
    <Transition name="slide">
      <div
        v-if="selectedGroup && !loading"
        class="flex flex-col flex-1 min-h-0 mt-3 overflow-hidden"
      >
        <!-- 命令组信息卡片 -->
        <div
          class="border border-border rounded-lg overflow-hidden bg-bg-secondary/30 flex flex-col flex-1 min-h-0"
        >
          <!-- 描述和标签 -->
          <div
            v-if="
            ...
            "
            class="px-3 py-2 border-b border-border/50 flex items-center gap-3 flex-wrap flex-shrink-0"
          >
            <p v-if="selectedGroup.description" class="text-xs text-text-muted">
              {{ selectedGroup.description }}
            </p>
            <div
              v-if="selectedGroup.tags && selectedGroup.tags.length > 0"
          ...
          <!-- 命令预览列表 -->
          <div
            class="font-mono text-xs overflow-y-auto scrollbar-custom bg-terminal-bg flex-1 min-h-0 py-1"
          >
            <div
            ...
            >
              <span class="text-text-muted/60 mr-2 select-none"
                >{{ idx + 1 }}.</span
              >
...
const emit = defineEmits<{
  "update:modelValue": [groupId: number];
...
async function loadGroups() {
  ...
  try {
    ...
    if (
      props.modelValue &&
      groups.value.some((g) => g.id === props.modelValue)
    ) {
