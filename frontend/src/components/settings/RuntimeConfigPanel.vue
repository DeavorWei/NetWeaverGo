<template>
  <div class="runtime-config-panel runtime-shell">
    ...
    <div class="runtime-settings-panels-flow">
      <!-- 超时配置 -->
      <div
        class="runtime-card bg-bg-card border border-border rounded-xl p-5 shadow-card settings-panel-card"
      >
        <h3
          class="text-sm font-semibold text-text-secondary mb-4 flex items-center gap-2"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >命令执行超时
              <HelpTip text="单条命令执行最长等待时间，默认 30000 毫秒。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >连接超时
              <HelpTip
                text="建立 SSH/SFTP 网络连接的最长等待时间，默认 10000 毫秒。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >握手超时
              <HelpTip text="SSH 协议握手阶段的最长等待时间，默认 10000 毫秒。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >短命令超时
              <HelpTip text="用于快速命令的超时阈值，默认 10000 毫秒。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >长命令超时
              <HelpTip
                text="用于大输出或慢设备命令的超时阈值，默认 60000 毫秒。"
            /></label>
            <input
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >每设备最大日志数
              <HelpTip
                text="单台设备缓存的最大日志条目数，超过后会按策略截断，默认 500。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >最大日志长度
              <HelpTip text="单条日志保留的最大字符数，默认 2000。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >日志截断阈值
              <HelpTip text="当日志接近上限时触发截断的百分比阈值，默认 95% 。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >最大并发设备数
              <HelpTip text="执行任务时同时处理的设备数量上限，默认 32。"
            /></label>
            <input
      ...
      >
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >工作协程数 <HelpTip text="引擎用于并发执行的协程数量，默认 10。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >事件缓冲区大小
              <HelpTip text="主事件队列容量，过小会增加阻塞风险，默认 1000。"
            /></label>
            <input
      ...
      >
        ...
        <div class="settings-auto-grid">
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >单设备超时（毫秒）
              <HelpTip text="单台设备从连接到采集完成的最长允许时间。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >命令超时上限（毫秒）
              <HelpTip
                text="发现命令执行超时上限，会约束命令 profile 超时配置。"
            /></label>
            <input
      ...
      >
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >最大推断候选数
              <HelpTip text="FDB 推断时候选设备超过该值将跳过推断，避免误判。"
            /></label>
            <input
      ...
      >
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >默认缓冲区大小
              <HelpTip text="通用场景的基础缓冲区大小，默认 4096 字节。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >小缓冲区大小
              <HelpTip text="轻量任务使用的小缓冲区大小，默认 1024 字节。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >大缓冲区大小
              <HelpTip text="大数据量输出场景使用的大缓冲区，默认 8192 字节。"
            /></label>
            <input
      ...
      >
        ...
        <div class="settings-auto-grid">
          <div class="space-y-2">
            <label class="settings-label"
              >行数阈值
              <HelpTip text="输出行数超过该阈值后触发分页检测，默认 50 行。"
            /></label>
            <input
          ...
          <div class="space-y-2">
            <label class="settings-label"
              >检测间隔（毫秒）
              <HelpTip
                text="分页检测轮询间隔，越小响应越快但开销更高，默认 100ms。"
            /></label>
            <input
    ...
    <div
      v-if="message"
      class="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-slide-up"
    >
      <div
...
async function loadConfig() {
  try {
    const data = await SettingsAPI.getRuntimeConfig();
    config.value = {
      timeouts: {
...
async function saveConfig() {
  ...
  try {
    ...
    await SettingsAPI.updateRuntimeConfig(data);
    originalConfig.value = JSON.stringify(config.value);
...
async function resetToDefault() {
  ...
  saving.value = true;
  try {
    await SettingsAPI.resetRuntimeConfigToDefault();
    await loadConfig();
    showMessage("已重置为默认值");
  } catch (err) {
