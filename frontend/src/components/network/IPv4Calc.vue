...
const parseMaskOrWildcard = (maskStrInput: string) => {
  let potentialCidr = maskStrInput.startsWith("/") ? maskStrInput.substring(1) : maskStrInput
  const cidrNum = parseInt(potentialCidr, 10)
  
  if (!isNaN(cidrNum) && String(cidrNum) === potentialCidr && cidrNum >= 0 && cidrNum <= 32) {
    return cidrNum
  }
...
const calculateSubnetDetails = (ipStr: string, cidr: number) => {
  ...
  return [
    ...
    { label: '子网范围', value: `${longToIp(networkLong)} - ${longToIp(broadcastLong)}` },
    { label: '可用主机数', value: usableHosts > 0 ? usableHosts.toLocaleString() : '0' },
    { label: '首个可用地址', value: usableHosts > 0 ? longToIp(firstUsableLong) : 'N/A' },
    { label: '最后一个可用地址', value: usableHosts > 0 ? longToIp(lastUsableLong) : 'N/A' }
  ]
}
...
const evaluation = computed(() => {
  ...
  if (cidr === -1) {
    return { error: '无效的掩码格式，请输入CIDR、子网掩码或反掩码', records: [] }
  }
...
const subnettingEvaluation = computed(() => {
  ...
  if (!ip || !mask || (!hStr && !sStr)) {
    return { error: null, warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
  }

  const cidr = parseMaskOrWildcard(mask)
  if (cidr === -1 || !isValidIp(ip)) {
    return { error: '请先完成上方基础网络信息的有效填写', warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
  }
  ...
  if (hStr) {
    ...
    if (isNaN(hosts) || hosts <= 0) {
      return { error: '请输入有效的主机数', warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
    }
    const hostBits = getRequiredHostBits(hosts);
    targetCidr = 32 - hostBits;
    if (targetCidr < cidr) {
      return { error: `当前掩码 /${cidr} 的网络无法提供 ${hosts} 个连续主机的空间`, warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
    }
  } else if (sStr) {
    const subnets = parseInt(sStr, 10);
    if (isNaN(subnets) || subnets <= 0) {
      return { error: '请输入有效的子网数', warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
    }
    const subnetBits = getRequiredSubnetBits(subnets);
    targetCidr = cidr + subnetBits;
    if (targetCidr > 32) {
      return { error: `当前掩码 /${cidr} 无法划分出 ${subnets} 个子网，位空间不足`, warning: null, showForceButton: false, subnets: [], totalSubnets: 0 }
    }
  }
  ...
  for (let i = 0; i < generateCount; i++) {
    ...
    const usableHosts = targetCidr < 31 ? Math.pow(2, 32 - targetCidr) - 2 : targetCidr === 31 ? 2 : 1;
  ...
  } else if (forceDisplayAllSubnets.value && totalSubnets > absoluteMaxSubnets) {
    warning = `数据量极其庞大！已强制展示前 ${absoluteMaxSubnets.toLocaleString()} 个子网。为防止浏览器崩溃，剩余数据不再渲染。`;
    showForceButton = false;
  } else if (forceDisplayAllSubnets.value && totalSubnets > maxSubnetsToGenerate) {
    warning = `已强制展示全部 ${totalSubnets.toLocaleString()} 个子网，由于节点众多，页面若有轻微卡顿属于正常现象。`;
    showForceButton = false;
  }

  return { error: null, warning, showForceButton, subnets: subnetsList, totalSubnets }
})
...
const exportToCSV = () => {
  ...
  // 1. 准备 CSV 表头
  const headers = ['序号', '网络号', 'CIDR', '首个可用 IP', '最后可用 IP', '广播地址', '子网掩码'];
  ...
  link.setAttribute('download', `${baseNet.replace(/\./g, '_')}_子网划分明细.csv`);
  ...
  <div class="space-y-6 md:space-y-8">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-col gap-2">
            ...
            <input 
              ...
              class="px-4 py-3 bg-bg-tertiary/50 border border-border rounded-xl focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all w-full font-mono text-text-primary placeholder-text-muted"
            />
          </div>
        </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="relative">
              <input 
                ...
                placeholder="例如: 50" 
                class="px-4 py-3 bg-bg-tertiary/50 border border-border rounded-xl focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all w-full font-mono disabled:opacity-50 disabled:cursor-not-allowed text-text-primary placeholder-text-muted"
              />
            <button 
              v-if="hostCountStr" 
              @click="hostCountStr = ''" 
    ...
    <transition name="fade">
      <div v-if="subnettingEvaluation.error" class="bg-error-bg text-error border border-error/30 p-4 rounded-xl flex items-center shadow-sm">
        ...
        <span class="text-sm font-medium">{{ subnettingEvaluation.error }}</span>
      </div>
    </transition>

    <!-- 划分子网结果展示 (排在计算结果之前) -->
    <transition name="fade">
      <section v-if="subnettingEvaluation.subnets.length > 0" class="bg-bg-secondary/60 glass border border-border rounded-2xl shadow-card p-5 md:p-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <button 
              @click="exportToCSV"
              class="flex items-center text-sm font-medium text-white bg-accent hover:bg-accent/90 px-4 py-1.5 rounded-full shadow-glow transition-all focus:ring-2 focus:ring-accent outline-none"
        ...
        <div v-if="subnettingEvaluation.warning" class="bg-warning-bg text-warning border border-warning/30 p-4 rounded-xl flex shadow-sm mb-5 text-sm items-start justify-between">
          <div class="flex items-start">
            ...
            <span class="leading-relaxed">{{ subnettingEvaluation.warning }}</span>
          </div>
          <button 
