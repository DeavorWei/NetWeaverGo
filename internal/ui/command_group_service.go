...
func (s *CommandGroupService) ListCommandGroups() ([]config.CommandGroup, error) {
	// 尝试迁移旧数据
...
func (s *CommandGroupService) GetCommandGroup(id string) (*config.CommandGroup, error) {
	return config.GetCommandGroup(id)
}

// CreateCommandGroup 创建新命令组
func (s *CommandGroupService) CreateCommandGroup(group config.CommandGroup) (*config.CommandGroup, error) {
	return config.CreateCommandGroup(group)
}

// UpdateCommandGroup 更新命令组
func (s *CommandGroupService) UpdateCommandGroup(id string, group config.CommandGroup) (*config.CommandGroup, error) {
	return config.UpdateCommandGroup(id, group)
}

// DeleteCommandGroup 删除命令组
func (s *CommandGroupService) DeleteCommandGroup(id string) error {
	return config.DeleteCommandGroup(id)
}

// DuplicateCommandGroup 复制命令组
func (s *CommandGroupService) DuplicateCommandGroup(id string) (*config.CommandGroup, error) {
	return config.DuplicateCommandGroup(id)
}

// ImportCommandGroup 从文件导入命令组
func (s *CommandGroupService) ImportCommandGroup(filePath string) (*config.CommandGroup, error) {
	return config.ImportCommandGroup(filePath)
}

// ExportCommandGroup 导出命令组到文件
func (s *CommandGroupService) ExportCommandGroup(id string, filePath string) error {
