...
- [NetWeaverGo 项目架构说明书](#netweavergo-项目架构说明书)
  ...
  - [4. 后端架构（Go）](#4-后端架构go)
    ...
    - [4.20 SNMP 查询模块 internal/snmp/](#420-snmp-查询模块-internalsnmp)
      - [4.20.1 架构分层](#4201-架构分层)
      - [4.20.2 查询器 Querier](#4202-查询器-querier)
      - [4.20.3 凭据加密](#4203-凭据加密)
      - [4.20.4 职责边界](#4204-职责边界)
      - [4.20.5 文件索引](#4205-文件索引)
    - [4.21 CEAS 硬件清单与批次预警模块 internal/ceas/](#421-ceas-硬件清单与批次预警模块-internalceas)
  - [5. 前端架构（Vue 3 + TypeScript）](#5-前端架构vue-3--typescript)
    ...
    - [5.6 SNMP 查询前端层](#56-snmp-查询前端层)
    - [5.7 组件体系](#57-组件体系)
    - [5.8 样式架构](#58-样式架构)
  - [6. 数据库设计](#6-数据库设计)
    - [6.1 数据库选型](#61-数据库选型)
    - [6.2 核心表结构](#62-核心表结构)
    - [6.3 SNMP 数据模型](#63-snmp-数据模型)
  - [7. 构建与部署](#7-构建与部署)
    - [7.1 构建流程](#71-构建流程)
...
|------|------|------|
| **Go** | >= 1.26 | 主语言 |
| **Wails v3** | v3.0.0-alpha.95 | 桌面应用框架（Go ↔ WebView 桥接） |
| **GORM** | v1.31.1 | ORM 框架 |
| **SQLite** (glebarez/sqlite) | v1.11.0 | 嵌入式数据库 |
| **golang.org/x/crypto** | v0.52.0 | SSH/SFTP 客户端实现 |
| **github.com/pkg/sftp** | v1.13.10 | SFTP 文件传输 |
| **github.com/gosnmp/gosnmp** | v1.43.2 | SNMP v1/v2c/v3 协议 |
| **github.com/robfig/cron/v3** | v3.0.1 | Cron 调度器（任务定时执行） |
| **github.com/fclairamb/ftpserverlib** | v0.30.0 | FTP 服务器 |
| **github.com/pin/tftp/v3** | v3.2.0 | TFTP 服务器 |
| **github.com/spf13/afero** | v1.15.0 | 文件系统抽象 |
| **golang.org/x/net** | v0.55.0 | 网络协议基础库 |
...
main()
  ...
  └── runGUI()                       # 启动 Wails GUI
        ...
        ├── ui.New*Service() × 16                   # 创建各 UI 服务实例
        ├── application.New()                       # 创建 Wails 应用
        │   └── Services: [16个 Wails Service]      # 注册服务到 Wails 桥接
        └── app.Run()                               # 启动主循环
```
...
- [`DeviceRepository`](internal/repository/interfaces.go:19) — 设备资产 CRUD + 分页查询 + 事务支持
- [`CommandGroupRepository`](internal/repository/interfaces.go:75) — 命令组 CRUD
- [`TaskGroupRepository`](internal/repository/interfaces.go:90) — 任务组 CRUD
...
**错误处理策略**（[`ErrorAction`](internal/executor/executor.go:21)）：
...
模板驱动的 CLI 输出解析系统（支持 regex / aggregate / tree 三大解析引擎）：

```
ParserManager (模板管理器，管理多厂商不可变只读快照)
├── Bootstrap()           # 加载内置模板（huawei/h3c/cisco）+ 用户自定义模板
├── GetParser(vendor)     # 获取厂商组合解析器（并发只读）
├── ReloadVendor(vendor)  # 热重载指定厂商（带事务回滚保障）
└── SetEngineMode(mode)   # 引擎模式控制 (auto / legacy_only / tree_only)

CompositeParser (组合解析器)
├── RegexParser           # 纯正则解析器（支持命名捕获组与索引捕获组）
├── Mapper                # 字段重命名映射器 (FieldMapping)
├── AggregateEngine       # 多行聚合状态机引擎 (RecordStart + Filldown)
└── TreeEngine            # 声明式二维规则树引擎（eDesk Pro 架构，支持嵌套与分块）
    ├── BlockSplitter     # 文本切块工具（保持块头，尾块自动收敛）
    └── ResultTile        # 树节点平铺展开器（编译期 IsPath 剪枝，笛卡尔积展开与字段补齐）
```

| 文件 | 职责 |
|------|------|
| [`manager.go`](internal/parser/manager.go) | [`ParserManager`](internal/parser/manager.go) — 模板管理器（内置与用户库合并，支持无内置纯自定义厂商，热加载失败防御） |
| [`composite_parser.go`](internal/parser/composite_parser.go) | 组合解析器（三引擎智能分发与字段映射） |
| [`regex_parser.go`](internal/parser/regex_parser.go) | 正则解析器（支持 (?P<name>...) 与数字索引） |
| [`aggregate_engine.go`](internal/parser/aggregate_engine.go) | 聚合引擎（多行记录边界状态机） |
| [`tree_engine.go`](internal/parser/tree_engine.go) | 二维规则树引擎（只读无竞态解析） |
| [`tree_rules.go`](internal/parser/tree_rules.go) | 规则树数据模型与 DAG 拓扑成环编译预检 |
| [`block_splitter.go`](internal/parser/block_splitter.go) | CLI 文本块切分器 |
| [`result_tile.go`](internal/parser/result_tile.go) | 树形结构扁平化平铺展开（字段对齐与默认值注入） |
| [`models.go`](internal/parser/models.go) | 解析器通用模型定义 |
| [`templates/builtin/`](internal/parser/templates/builtin/) | 嵌入式内置模板库（huawei.json / h3c.json / cisco.json） |
| [`tools/re2scan/`](tools/re2scan/main.go) | Go RE2 正则兼容性静态扫描器（支持前瞻/后顾/反向引用识别与迁移建议） |

#### 4.11.1 设备形态认知引擎 internal/device/ (P2 认知层)

[`internal/device/`](internal/device/) 实现了高内聚、无副作用的纯设备形态与版本认知体系：

```
                    命令原始回显 (version / patch_info / device / sysname)
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                       device.Identify(vendor, raws)                         │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │
         ┌─────────────────────────────┼─────────────────────────────┐
         ▼                             ▼                             ▼
┌─────────────────┐           ┌─────────────────┐           ┌─────────────────┐
│ identifyHuawei  │           │   identifyH3C   │           │  identifyCisco  │
│ - _REG2HANDLER  │           │ - Comware 识别  │           │ - IOS / IOS-XE  │
│ - VRP / VRBD    │           │ - 款型/版本解析  │           │ - 款型/版本解析  │
└────────┬────────┘           └────────┬────────┘           └────────┬────────┘
         │                             │                             │
         └─────────────────────────────┼─────────────────────────────┘
                                       │
                       ...
                       ┌───────────────────────────────┐
                       │       device.Identity         │
                       │   Model / Series / Version    │
                       │   Patch / Evidence (证据链)   │
                       └───────────────────────────────┘
```

- **单向解耦桥接**：保持纯识别引擎无外部拓扑或数据库依赖，拓扑执行引擎单向调用 `device.Identify` 并映射为 `parser.DeviceIdentity`。
- **系列归一化算法**：[`series.go`](internal/device/series.go) 严格对齐 eDeskPro 规范，支持 E6/AR/AC/AP/CE/FM/S/AirEngine/NE/USG 等各大产品线的主系列规整。
- **全链路持久化**：识别结论通过 `SaveDeviceIdentity` 同步更新 `task_run_devices` 和 `device_assets`，并向前端提供证据链审计与画像匹配路径展示。
...
- 解析规划链路
- 与实际拓扑边进行匹配
- 生成差异报告（missing_link / device_mismatch / aggregation_mismatch / interface_mismatch / unexpected_link / one_side_only）
- 导出 json / csv / html 报告
...
- [`BatchPingEngine`](internal/icmp/engine.go:15) — 并发 Ping 引擎
- [`DefaultPingConfig`](internal/icmp/types.go) — 默认探测配置（`Count=1`, `Concurrency=64`, `Timeout=1s`, `PacketSize=32`，该参数与单元测试断言严格对齐，保证高效轻量探测）
- 可配置并发数（最大 256）、超时、数据包大小、次数
- 进度回调机制
- [`icmp_windows.go`](internal/icmp/icmp_windows.go) — Windows 原生 ICMP 实现 (IcmpSendEcho)
...
| [`parse_template_service.go`](internal/ui/parse_template_service.go) | 解析模板管理服务（CRUD、热加载、Reload失败自动回滚、字段防覆盖、在线回显测试） |
| [`snmp_query_service.go`](internal/ui/snmp_query_service.go) | SNMP 即时查询与凭据管理服务 |
...
### 4.20 SNMP 查询模块 internal/snmp/

SNMP 查询模块面向**交付现场的一次性采集场景**：设备上架后的上线确认、资产信息（型号/版本/序列号）核对。

模块刻意**不提供**周期性轮询调度、Trap 值守监听和 MIB 库管理 —— 这些属于长期值守型网管系统的职责，交付工具承担会带来部署负担（固定 IP、162 特权端口长期占用、数据留存策略），却无法替代专业 NMS。
...
前端 SNMPQuery.vue
  ...
  → Wails 绑定 internal/ui/snmp_query_service.go (SNMPQueryService)
      → internal/snmp.Querier.Get / Walk / GetDeviceInfo / BatchGetDeviceInfo
          → Querier.connect() 分发到 connectV1V2 / connectV3
          → gosnmp.Get / Walk
...
| 层 | 组件 | 职责 |
|----|------|------|
| UI 服务层 | `SNMPQueryService` | 参数校验、凭据解析、视图模型转换、超时控制 |
| 核心层 | `Querier` | SNMP 客户端构造、GET/WALK 执行、PDU 结果转换、批量并发控制 |
| 安全层 | `CredentialCrypto` | AES-256-GCM 凭据加解密、密钥文件管理 |
| 数据层 | `CredentialRepository` | 凭据持久化（独立库 `snmp.db`） |

#### 4.20.2 查询器 Querier

`Querier` 无状态、无后台任务，四个对外能力：

| 方法 | 说明 |
|------|------|
| `Get(ctx, addr, oids, cred)` | 对多个 OID 执行 GET |
| `Walk(ctx, addr, rootOID, cred)` | 对指定根 OID 执行 WALK |
| `GetDeviceInfo(ctx, addr, cred)` | 采集 `sysDescr` / `sysUpTime` / `sysName` |
| `BatchGetDeviceInfo(ctx, addrs, cred)` | 批量并发采集，用于上线确认 |

**设备信息采集容错**：逐个采集三个默认 OID，单个失败不影响其余（失败项保留行位并标注 `Error`），仅当全部失败时才判定设备不可达。这样即使设备未配置 `sysName`，仍可取得 `sysDescr` 用于版本核对。

**批量并发控制**：使用 channel 信号量而非 worker pool，并发上限取自 `QuerierConfig.Concurrency`（默认 10）。每台设备一个 goroutine 互相隔离，结果按入参 index 回填，前端无需再做映射；全流程受 `context` 控制（总超时 10 分钟）。

**结果转换**：`formatPDUValue` 处理 OctetString（可打印转字符串否则十六进制）、TimeTicks（转为 `ticks (duration)`）与各类异常类型；`getPDUTypeString` 映射 PDU 类型到可读名称。

#### 4.20.3 凭据加密

- 敏感字段（`Community` / `AuthPassword` / `PrivPassword`）以 AES-256-GCM 加密落库，密文带 `ENC1:` 前缀以便可靠识别；
- 写入时仅当字段非空且尚未加密时才加密，保证幂等；
- 读取时识别前缀后解密，为空则回落默认 community `public`；
- 列表查询不返回敏感字段明文，更新时敏感字段留空表示保持原值不变；
- 密钥文件 `snmp_key.bin` 存于 SNMP 数据目录，需与数据一并备份。

#### 4.20.4 职责边界

| 职责 | 是否提供 |
|------|----------|
| 单次 GET / WALK、设备信息采集、批量上线确认、凭据管理 | ✅ |
| 周期性轮询、趋势图、历史留存 | ❌ 交由专业监控系统 |
| Trap 接收与告警 | ❌ 交由专业监控系统 |
| MIB 文件导入与 OID 树浏览 | ❌ OID 含义查阅外部文档 |

#### 4.20.5 文件索引

| 文件 | 职责 |
|------|------|
| `querier.go` | 查询器：客户端构造、GET/WALK、设备信息与批量采集、结果转换 |
| `types.go` | 结果类型与加密器结构定义 |
| `crypto.go` | AES-256-GCM 加解密实现 |
| `../models/snmp.go` | `SNMPCredential` 模型 |
| `../repository/credential_repository.go` | 凭据数据访问 |
| `../config/snmp_db.go` | SNMP 凭据库初始化与迁移 |
| `../ui/snmp_query_service.go` | Wails UI 服务层 |

### 4.21 CEAS 硬件清单与批次预警模块 internal/ceas/

#### 4.21.1 模块定位
CEAS（Customer Electronic Asset Survey）模块承载网络设备电子标签（`display elabel`）层级化解析与 BOM 批次安全隐患联动预警。采用零 Python 依赖的纯 Go 专用解析器，构建完整的机框、槽位、板卡、端口物理拓扑树。

#### 4.21.2 核心能力
1. **独立专用解析器（`internal/ceas/elabel.go`）**：
   - 10 类节点有序优先表匹配（frame / fanframe / power / mainboard / motherboard / daughterboard / ofccard / port / card / slot）；
   - `handle_extra_properties` 属性块内多段自动拆分（生成独立子卡节点）；
   - `last_slot` 槽位上下文继承机制。
2. **三段式物理位置生成（`internal/ceas/hierarchy.go`）**：
   - 统一生成标准三段式 ID（`Slot_X → X/0`、`Sub_Board_X/Y → X/Y`、`Port → X/Y/Port`）及层级 ID（`0_1_2`）。
3. **ESN 序列号提取（`internal/ceas/esn.go`）**：
   - 覆盖 CE、AR、WLAN 及通用多策略回退，自动回写设备资产库与运行记录。
4. **BOM 批次预警比对（`internal/ceas/bom.go`）**：
   - 内置 36 条高危与重点关注物料批次预警种子（涵盖 CE/S/AR/NE/WLAN/FW 6 大产品家族），支持用户自定义扩展与动态启停，输出精确至"设备 × 槽位 × 物理位置"的预警矩阵。
5. **执行运行时对接（`internal/taskexec/ceas_compiler.go` & `ceas_executor.go`）**：
   - 支持 `RunKindCEAS` 与 `StageKindCEASCollect`，包含 `diagnose` 提权视图兜底（`quit` → `return` 安全退出），生成 4 大标准产物（`ceas_cmdecho` / `ceas_data` / `ceas_baseinfo` / `ceas_alerts`），数据库操作事务隔离防历史节点膨胀。
6. **Wails 服务层（`internal/ui/hardware_inventory_service.go`）**：
   - 提供硬件树查询、懒加载子节点、BOM 观察清单增删改查、CSV 报表导出及即时任务下发。

### 4.22 设备巡检与合流模块 internal/inspection/

#### 4.22.1 模块定位
巡检应用层与交付闭环合流模块是网络自动化日常巡检、割接质量验收以及 A2 合规性审计与 A5 交付测试的统一结论载体。提供纯 Go 原生的高性能外置规则判定引擎、前置采集优化及标准化报表导出。

#### 4.22.2 核心能力
1. **统一测试结论载体（`internal/inspection/result.go`）**：
   - 统一定义标准 8 种测试结论码（`TEST_PASS`、`TEST_FAIL`、`TEST_WARNING`、`TEST_UNACCORD`、`TEST_IGNORE`、`TEST_EXCEPT`、`TEST_MANUAL`、`TEST_UNTEST`）与 4 级严重度（`blocker`、`major`、`minor`、`info`），规范化承载后续 A2/A5 结论映射；支持历史别名 `"critical"` 安全归一化防御；
2. **多模判定引擎（`internal/inspection/engine.go`）**：
   - 支持数值闭区间合格（`bound`）、区间外合格（`outside`）、数值小于等于（`lte`）、数值大于等于（`gte`）、全等（`equals`）、必须包含（`must_contain`）、禁止包含（`must_not_contain`）与正则表达式捕获（`regex`）；
   - 全面支持多行表格（Multi-row table）逐行遍历判定，任一行违规即时检出；支持空阈值规则主动防御返回 `TEST_EXCEPT`；
   - 异常时自动提取 CLI 回显命中行及 $\pm 1$ 行上下文证据（`EvidenceJSON`），并注入来自 eDesk Pro 专家库的处置建议（`Advice`）；
3. **编排与会话缓存优化（`internal/taskexec/inspection_compiler.go` & `inspection_executor.go`）**：
   - 任务级编译器优先排序 `IsPreCollect=true` 项；执行器对检查项命令去重并在设备级会话内缓存，杜绝重复向现网设备发送相同 CLI 查询；
   - 执行器采用 `(run_id, device_ip)` 联合条件幂等清理，实现多轮任务执行结果严格物理隔离；
4. **内置知识库种子（`internal/inspection/seeds.go`）**：
   - 内置 4 套模板（General, CE, S, AR）及 12 条高价值检查项规则，覆盖 CPU、内存、环境温度、风扇电源、接口错包、OSPF 与 BGP 对等体；
5. **报表导出（`internal/inspection/export.go`）**：
   - 支持带 UTF-8 BOM（`\xEF\xBB\xBF`）的 CSV 格式导出（含举证追踪与上下文列，防 Excel 乱码）与格式化 JSON 导出；
6. **Wails 服务层与前端工作台（`internal/ui/inspection_service.go` & `frontend/src/views/Inspection.vue`）**：
   - 联动展示巡检指标看板（覆盖 8 种评估状态与设备状态）、多轮隔离批次下拉切换、矩阵过滤、规则库管理、发起向导与举证详情抽屉。
...
│       └── _index.css         # 工具类入口
│
└── views/                     # 页面视图组件（类型定义来自 bindings/，无独立 types/ 与 utils/ 目录）
    ├── Devices.vue            # 设备管理
    ├── Commands.vue           # 命令组管理
    ├── Tasks.vue              # 任务组管理
    ...
    ├── Settings.vue           # 系统设置
    ├── SNMP/                  # SNMP 查询模块
    │   └── SNMPQuery.vue      # 查询界面（设备查询/批量上线确认/凭据管理）
    └── Tools/
...
export const SNMPQueryAPI = {
  query,                  // 单次查询（device_info / get / walk）
  batchQueryDeviceInfo,   // 批量上线确认
  getDeviceInfoOIDs,      // 设备信息 OID 列表
...
SNMP 查询为**一过性同步操作**（发起请求 → 等待返回 → 渲染结果），不需要 EventBus 推送，因此不设独立的 Composable 层，逻辑内聚在视图组件中。

| 层次 | 文件 | 职责 |
|------|------|------|
| 视图 | [`frontend/src/views/SNMP/SNMPQuery.vue`](frontend/src/views/SNMP/SNMPQuery.vue) | 三个选项卡：设备查询 / 批量上线确认 / 凭据管理 |
| API | [`frontend/src/services/snmpApi.ts`](frontend/src/services/snmpApi.ts) | 封装 `snmpqueryservice` 绑定调用与请求构造 |
| 类型 | [`frontend/src/types/snmp.ts`](frontend/src/types/snmp.ts) | 复用绑定模型再导出，并定义操作与协议选项常量 |

**视图状态设计**：

```typescript
// 单台查询
queryAddress, queryOperation, queryOID
queryCredentialId | tempCredential   // 已保存凭据 / 一次性临时凭据二选一
queryResults, queryLatency

// 批量上线确认
batchAddressInput → batchAddressList  // 换行/逗号/空格分隔
batchResults, batchReachableCount

// 凭据编辑
credentialForm, credentialEditing     // 编辑时敏感字段清空，留空表示保持原值
```

**关键取舍**：批量结果表格直接按入参顺序展示，并通过 `extractValue(results, "sysName")` 从结果数组中取值，避免前端维护额外的索引结构。
...
| SNMPQuery | 查询表单、结果表格、批量结果表格、凭据管理 | SNMP 即时查询 |
...
SNMPCredential {
  ...
  string Community          // v1/v2c community（加密存储）
  string SecurityLevel      // noAuthNoPriv / authNoPriv / authPriv
  string Username           // v3 用户名
  string AuthProtocol       // MD5 / SHA / SHA224 / SHA256 / SHA384 / SHA512
  string AuthPassword       // 加密存储
  string PrivProtocol       // DES / AES / AES192 / AES256 / AES192C / AES256C
  string PrivPassword       // 加密存储
  string ContextName
  string ContextEngineID
}
```

SNMP 查询结果为**内存态数据**，不落库，因此不存在结果表与关联关系。
...
运行时通过 [`fs.Sub`](cmd/netweaver/main.go:138) 提取子文件系统，由 Wails 的 `AssetFileServerFS` 提供静态文件服务。
...
- [`DeviceRepository`](internal/repository/interfaces.go:19) — 数据访问
...
用户填写地址/操作/OID/凭据
    │
    ▼
SNMPQuery.vue 调用 snmpApi.query(req)
    │
    ▼
SNMPQueryService.Query(req)
    │
    ├── resolveCredential()      凭据 ID 优先，其次临时凭据
    ├── context.WithTimeout(30s)
    │
    ▼
Querier.connect()
    ├── getCommunity() → CredentialCrypto.DecryptCredential()
    └── createV1V2Client() / createV3Client()
    │
    ▼
按 operation 分派
    ├── device_info → GetDeviceInfo() 逐个采集三个默认 OID
    ├── get         → Get()
    └── walk        → Walk()
    │
    ▼
...
批量地址列表（换行/逗号/空格分隔）
    │
    ▼
SNMPQueryService.BatchQueryDeviceInfo(req)
    │
    ▼
Querier.BatchGetDeviceInfo(ctx, addrs, cred)
    │
    ├── channel 信号量限流（默认并发 10）
    ├── 每台设备一个 goroutine，互相隔离
    └── 结果按入参 index 回填
    │
    ▼
返回 SNMPBatchQueryResponse（reachable / results / latencyMs）
```

**关键特性**：查询结果为内存态数据，不写入数据库，因此不存在历史留存与清理流程。
...
SNMPQuery.vue → snmpApi.ts → Wails Bindings (snmpqueryservice)
SNMPQuery.vue → types/snmp.ts
```
