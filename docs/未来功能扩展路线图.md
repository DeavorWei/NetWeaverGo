...
| 功能 | 状态 | 说明 |
|------|------|------|
| SNMP GET/WALK | ✅ 已落地 | `internal/snmp/` + `SNMPQueryService`，支持 v1/v2c/v3，即时查询不落库 |
| 设备上线确认 | ✅ 已落地 | `Querier.BatchGetDeviceInfo` 批量并发采集 sysDescr/sysUpTime/sysName |
| SNMP 设备发现 | 🔜 待评估 | 通过 SNMP 批量识别设备类型与邻居，可作为拓扑发现的补充数据源 |
| SNMP Trap 接收 | ❌ 不做 | 属于长期值守型网管（Zabbix/Prometheus）职责，交付工具不承担 |
| MIB 浏览器 | ❌ 不做 | OID 含义查阅外部文档，不内置 MIB 解析与管理 |

**技术选型**：
- 使用 `github.com/gosnmp/gosnmp` — Go 最流行的 SNMP 库
- 支持 SNMPv1/v2c/v3，凭据 AES-256-GCM 加密存储
