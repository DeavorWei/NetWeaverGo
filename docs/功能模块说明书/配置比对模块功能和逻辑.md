...
1. **导入流程**：用户输入CSV路径 → CSV解析（直接读取原文件，不复制） → 接口名标准化 → 去重处理 → 数据库持久化
...
| `ID` | uint | 主键 | 自增 |
| `FileName` | string | 文件名 | - |
| `FilePath` | string | 导入时传入的原始 CSV 文件路径（不复制） | - |
...
| `aggregation_mismatch` | 聚合口成员不一致 | 低 |
...
**支持的列名格式**（设备名均为可选，接口为必填）：
| 标准列名 | 支持的别名 |
|----------|-----------|
| 本端设备名 | 本端设备、A设备、A设备名、源设备名称 |
| 本端管理IP | 本端IP、A管理IP、源设备IP |
| 本端接口 | A接口、源接口 |
| 对端设备名 | 对端设备、B设备、B设备名、目标设备名称 |
| 对端管理IP | 对端IP、B管理IP、目标设备IP |
| 对端接口 | B接口、目标接口 |
| 链路类型 | 类型 |
| 备注 | 备注 |

> 说明：设备名**非必填**，缺失时退化为 `name:<norm>` 或 `unknown`；IP 格式非法时仅追加 `warnings` 告警、**不阻断导入**；无向键生成时对端点做**小写化并去除空格**处理（如 `sw1:ge0/1<->sw2:ge0/2`）。
...
- 输出：`sw1:ge0/1<->sw2:ge0/2`（端点先做小写化、去除空格后拼接；与 `SW2:GE0/2<->SW1:GE0/1` 归一为同一键）
  ...
  └─> PlanCompareAPI.importPlanCSV(path)
        └─> PlanCompareService.ImportPlanCSV(ctx, path)
              └─> plancompare.Service.ImportPlanCSV(path)
                    ├─> os.Stat(path)                    // 检查文件
                    ├─> csv.NewReader(f).ReadAll()        // 读取CSV（直接读取原路径）
