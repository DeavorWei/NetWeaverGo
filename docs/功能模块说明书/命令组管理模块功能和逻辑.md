...
2. **创建流程**：用户填写表单 → 前端解析命令文本 → 调用 [`CreateCommandGroup()`](internal/config/command_group.go:56) → 写入数据库（名称唯一性由 GORM uniqueIndex 约束保证）
...
> 注：名称唯一性实际由 GORM `uniqueIndex` 约束在数据库层强制，并非显式 `SELECT COUNT` 预检；冲突时由数据库返回唯一约束错误后向上抛出，前端提示「名称已存在」。
...
sequenceDiagram
    ...
    DB-->>C: 返回源命令组
    C->>C: 复制结构体，ID = 0
    C->>C: Name = Name + " (1)"   # 初次复制后缀，冲突时循环改为 (2)/(3)...
    loop 名称冲突检测
        C->>DB: SELECT COUNT WHERE name = ?
        DB-->>C: 返回计数
        alt 名称已存在
