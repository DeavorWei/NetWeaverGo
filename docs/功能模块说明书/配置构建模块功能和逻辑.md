...
│  └─────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 语法展开函数集 (syntax_expander.go)                        │   │
│  │ - 范围语法展开 (1-10)  [ExpandSyntaxSugar]                 │   │
│  │ - 等差数列检测与推断 [Detect/InferArithmeticSequence]       │   │
│  │ - 变量值解析 [ParseVariableValues / ParseVariables]         │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
| **语法展开函数集** | `internal/forge/syntax_expander.go` | 语法糖展开、等差数列推断、变量解析（包级函数，非独立类型） |
...
| `Total` | int | 生成的配置块总数 |
| `Warnings` | []string | 警告信息（`Build()` 当前不写入；主要由 `ExpandValues()` 填充） |
...
sequenceDiagram
    ...
    participant Expander as 语法展开函数集
...
for i := 0; i < maxLen; i++ {
    ...
    for _, v := range sortedVars {
        ...
        currentBlock = strings.NewReplacer(v.Name, v.Values[valIndex]).Replace(currentBlock)
    }
...
ConfigBuilder.Build()
    ↓
syntax_expander.ParseVariables()
    ↓
syntax_expander.ExpandSyntaxSugar() [循环]
    ↓
syntax_expander.InferArithmeticSequence() [可选]
    ↓
...
ConfigBuilder.ExpandValues()
    ↓
syntax_expander.ParseVariableValues()
    ↓
syntax_expander.ExpandSyntaxSugar()
    ↓
syntax_expander.InferArithmeticSequence() [可选]
    ↓
ExpandResult 返回前端
...
| **语法糖展开** | 范围语法自动展开，等差数列补全 | syntax_expander（包级函数） |
