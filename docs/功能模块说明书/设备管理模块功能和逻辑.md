...
| `Password` | string | 登录密码 | 可选，明文存储（仅做首尾空白去除，无加密） |
| `Protocol` | string | 连接协议 | SSH/TELNET（SNMP 仅作分类标记；连接抽象层当前仅实现 SSH/Telnet，SNMP 设备不直接连通，查询走独立 SNMP 模块） |
...
**设计要点**：
- 仅在编辑场景返回数据库存储的明文密码（设备密码未加密，用于编辑回填）
...
**设计原则**：
1. 列表接口不返回密码字段
2. 详情接口返回数据库存储的明文密码（设备密码未加密，用于编辑回填）
3. 更新时使用密码合并规则

**密码合并规则** ([`config.MergePassword()`](internal/config))：仅当新密码为空字符串时保留旧值；传入非空（含单个空格）即视为新密码直接覆盖。
...
> 注意：清空密码必须传入**空字符串**；若前端以空格表示清空需先 trim 再传空，否则单个空格会被当作非空新密码存为 `" "`。设备模块**不提供 CSV 导入/导出**（`inventory.csv` 相关常量已弃用未使用），批量能力依靠 IP 范围展开与 `AddDevices / UpdateDevices / DeleteDevices` 实现。
...
| **密码处理** | 列表不返回密码，详情返回明文密码（未加密），更新时按空值保留/非空覆盖规则合并 |
