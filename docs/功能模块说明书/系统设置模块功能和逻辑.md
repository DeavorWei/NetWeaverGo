...
type RuntimeConfigData struct {
    ...
    Topology struct {
        ...
        InferenceWorkerCount   int `json:"inferenceWorkerCount"`   // 推理工作协程数
        InferenceDeviceTimeout int `json:"inferenceDeviceTimeout"` // 单设备推理超时（毫秒）
    } `json:"topology"`
    Buffers struct {
...
## 6. 设备画像与四级回退匹配机制 (P2 增强)

### 6.1 画像匹配模型架构

系统引入标准化设备画像（`DeviceProfile`）与四级回退解析机制，统一管理各厂商、系列与具体款型的命令画像、提示符规范与采集策略：

```
┌────────────────────────────────────────────────────────┐
│                   ResolveProfile                       │
│    (输入: vendor, model/series, version)              │
└──────────────────────────┬─────────────────────────────┘
                           │
             ┌─────────────▼─────────────┐
             │ Level 1: 精确款型/版本匹配  │  matchPath: exact:<model>
             │  (Selector.Match 预编译正则)│
             └─────────────┬─────────────┘
                           │ 未命中
             ┌─────────────▼─────────────┐
             │ Level 2: 产品系列匹配      │  matchPath: series:<series>
             │  (Selector.Series 匹配)    │
             └─────────────┬─────────────┘
                           │ 未命中
             ┌─────────────▼─────────────┐
             │ Level 3: 厂商基础画像      │  matchPath: vendor:<vendor>
             │  (Selector == nil 的主画像)│
             └─────────────┬─────────────┘
                           │ 未命中
             ┌─────────────▼─────────────┐
             │ Level 4: 全局保守兜底画像  │  matchPath: global:default
             │  (高兼容性通用配置)         │
             └───────────────────────────┘
```
...
1. **嵌入式细分规则库**：通过 Go 1.16+ `//go:embed profiles/*.json` 机制将各细分系列画像（如 `huawei_ce.json` 覆盖 CE6800/CE12800 系列、`huawei_s.json` 覆盖 S5700/S6700 系列）编译进二进制，启动时自动注册进画像注册表。
2. **注册优先级保证**：细分定制画像注册时通过前置插入机制，保证其匹配优先级高于厂商全局默认画像。
3. **预编译与高效匹配**：`CommandCondition` 与 `ProfileSelector` 在初始化与注册阶段一次性完成正则表达式编译（`modelRegex` / `versionRegex`），彻底消除执行期每条命令下发即时编译的开销。
4. **单向解耦桥接**：设备形态认知引擎（`internal/device`）保持纯函数化与领域独立；执行层与拓扑层通过 `MapCommandOutput` 与 `SaveDeviceIdentity` 单向接收其识别结论，回写资产表与任务运行设备的 `model`、`model_series`、`version`、`patch_version`、`profile_match_path` 及 `identity_evidence`。
