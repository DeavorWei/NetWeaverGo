...
type TaskGroup struct {
    ...
    OnceScheduledAt        *time.Time                 `json:"onceScheduledAt"`
    NextRunAt              *time.Time                 `json:"nextRunAt"`
    LastScheduledRunID     string                     `json:"lastScheduledRunId"`
...
| `CronExpression` | string | Cron 表达式（5 字段：分/时/日/月/周） | 调度类型为 cron 时使用 |
| `ScheduleType` | string | 调度类型 | `cron` / `once` |
| `OnceScheduledAt` | *time.Time | 一次性调度时间 | 调度类型为 once 时使用 |
| `NextRunAt` | *time.Time | 下次运行时间 | 由调度器计算回写 |
| `LastScheduledRunID` | string | 上次调度产生的运行 ID | 可选 |
| `LastScheduledAt` | *time.Time | 上次调度时间 | 可选 |
| `ScheduleError` | string | 最近调度错误信息 | 可选 |
...
任务模板支持后台定时调度，由独立调度子系统实现（`internal/taskexec/scheduler.go`、`schedule_validator.go` 与 `internal/ui/schedule_service.go`）：

- **调度类型**（`ScheduleType`）：`cron`（周期）与 `once`（一次性）。
- **Cron 表达式**：5 字段 `分 时 日 月 周`，经 `robfig/cron/v3` 解析校验；`ValidateCronExpression` 校验合法性，`DescribeCronExpression` 生成中文描述。
- **预设**：每 5/15/30 分钟、每小时、每天凌晨 2 点、工作日 9 点、每周一 3 点、每月 1 号等（`GetSchedulePresets`）。
- **启用 / 禁用**：`EnableSchedule` / `DisableSchedule`；禁用失败时数据库回滚，保证配置一致。
- **运行时间回写**：`NextRunAt` 由调度器计算并回写；`LastScheduledRunID` / `LastScheduledAt` 记录最近一次调度结果，`ScheduleError` 记录异常。
- **状态查询**：`GetSchedulerStatus` 查看调度器运行状态，`ListScheduleLogs` 查看调度日志。

> 注意：定时调度仅在应用运行期间生效，依赖常驻的调度器进程。
