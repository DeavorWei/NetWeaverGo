...
- **现状**：`DeviceAsset.Password` 明文入库，虽前端接口已脱敏，但数据库存储仍存在安全风险
- **影响**：数据库文件被复制或备份泄露时，设备凭据暴露
- **证据**：`internal/models/models.go` 中 `DeviceAsset.Password` 为明文字段
...
// NewCredentialCipher 创建加密器
func NewCredentialCipher() (*CredentialCipher, error) {
...
func getOrCreateKey() ([]byte, error) {
	// 密钥存储路径
	keyPath := filepath.Join(GetDataDir(), ".secret", "credential.key")
	...
	// 生成新密钥 (AES-256 需要 32 字节)
	key := make([]byte, 32)
	if _, err := io.ReadFull(rand.Reader, key); err != nil {
		return nil, fmt.Errorf("生成密钥失败: %v", err)
	}

	// 保存密钥 (权限 0600：仅所有者可读写)
