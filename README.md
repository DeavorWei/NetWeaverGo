...
### 🔍 设备巡检与合流
- 4 套内置模板 + 12 条专家检查项（CPU/内存/温度/风扇/电源/接口/时钟/协议）
- 单阶段与三阶段编排（采集 → 解析 → 判定，采集完成即释放设备连接）
- 多语言报表导出（中文 / English）、检查项分组树、举证与上下文追因

</td>
<td width="50%">

### 📈 运行可观测性
- RunID 分桶指标（缓存命中、风险命令、设备形态处理器、巡检结果码等）
- 执行详情页「本次运行指标」卡片，随运行快照实时刷新
- 升级前主库自动备份与保留轮转（失败仅告警、不阻断启动）
...
- [`docs/功能模块说明书/`](docs/功能模块说明书/) — 18 份功能模块详细文档，涵盖设备管理、任务执行、巡检、解析模板、拓扑发现、SNMP 查询、配置生成、拓扑命令配置等所有模块
- [`docs/全量代码审计报告.md`](docs/全量代码审计报告.md) — 能力升级实施的全量代码审计（含闭环状态标注）
- [`docs/全量代码审计报告-分阶段修复方案.md`](docs/全量代码审计报告-分阶段修复方案.md) — 审计遗留项的分阶段修复方案与执行记录
